...
let layoutOrgMatrix = {
    ...
    createFormHeader: function () {
        ...
        layout.form.forEach(element => {
            ...
            FormHeader += `
            <div id="prod-${element.id}" aria-hidden="true"
                ...
                layout.form.forEach(item => {
                    ...
                    $.ajax({
                                ...
                                success : function (res2){
                                    ...
                                    let val;
                                    if(data){
                                        val =`<img src='${APP_URL}/assets/icon/Icon-CheckSphare-Fill.svg'>
                                        <a href="javascript:void(0)" data-tjid="${data.id}" class="setting-limit"><img src="${APP_URL}/assets/icon/Icon-Data-Transaction.svg" height="34"></a>
                                        `;
                                        ...
                                        val =`<img src='${APP_URL}/assets/icon/Icon-Not-Fill.svg'>`;
                                    }
                                    let bgcolor = item.id == prodid ? "#faf590" : "#fff";
                                    matrix += `
                                    <div class="${classParent} pointer"

                                        style="background-color:${bgcolor};width:calc(${layout.defaultWidthHeader}px / ${layout.form.length}); height:51px;">${val}
                                    </div>`;
                                },error : function (){}
                            })
                })
...
let utils = {
    ...
    getListSop: function() {
        $.ajax({
            url: `${APP_URL}/bussines/corporate/list-item`,
            type: "GET",
            dataType: "json",
            async: false,
